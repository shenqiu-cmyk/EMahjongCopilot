...
from mjmax.proto import config_pb2, sheets_pb2, basic_pb2
from liqi_proto import liqi_pb2
from google.protobuf import json_format
from .update_liqi import get_version
from common.log_helper import LOGGER

MOD_CONFIG_FILE_PATH = r'./mjmax/config/settings.mod.yaml'
LQC_LQBIN_FILE = r"./mjmax/proto/lqc.lqbin"
...
''')
        try:
            with open(MOD_CONFIG_FILE_PATH, 'r', encoding='utf-8') as f:
                temp = YAML()
            ...
            LOGGER.error(f'未检测到mod配置文件，已生成默认配置，如需自定义mod配置请手动修改 {MOD_CONFIG_FILE_PATH}', e,
                         exc_info=True)
        ...
        if self.settings['resource']['auto_update']:
            ...
            try:
                ...
                LOGGER.error('更新lqc.lqbin文件失败！可能会导致角色和物品不全！', e, exc_info=True)

        with open(LQC_LQBIN_FILE, 'rb') as f:
            self.load_lqc_lqbin(f.read())
        self.SaveSettings()

    def SaveSettings(self):
        with open(MOD_CONFIG_FILE_PATH, 'w', encoding='utf-8') as f:
    ...
    def update_resource(self):
        ...
        if self.settings['resource']['lqc_lqbin_version'] == prefix:
            LOGGER.info(f'lqc.lqbin文件无需更新，当前版本：{prefix}')
        else:
            # 更新lqc.lqbin
            lqc_lqbin = self.get_lqc_lqbin(prefix)
            with open(LQC_LQBIN_FILE, 'wb') as f:
                f.write(lqc_lqbin)
            self.settings['resource']['lqc_lqbin_version'] = prefix
            LOGGER.info(f'lqc.lqbin文件更新成功：{prefix}')
    ...
    def main(self, message, liqi_proto):
        ...
        buf = message.content
        msg_type = liqi.MsgType(buf[0])
        msg_block = basic_pb2.BaseMessage()
        if msg_type == liqi.MsgType.NOTIFY:
            # Notify没有msg_id
            ...
            if msg_type == liqi.MsgType.REQ:
                ...
                match method_name:
                    ...
                    case '.lq.Lobby.changeCharacterSkin':  # 修改角色皮肤
                        ...
                        inject_msg = b'\x01' + \
                                     liqi.toProtobuf(update_data_block)
                    ...
                    case '.lq.Lobby.setRandomCharacter':
                        ...
                        self.settings['config']['random_character']['pool'] = \
                            json_format.MessageToDict(data, including_default_value_fields=True,
                                                      preserving_proto_field_name=True)['pool']
                        self.SaveSettings()
            ...
            elif msg_type == liqi.MsgType.RES:
                # Res类型必定是客户端收到的消息
                ...
                match method_name:
                    ...
                    case '.lq.Lobby.login' | '.lq.Lobby.oauth2Login':  # 登录时
                        ...
                        if self.settings['config']['character'] in self.settings['config']['characters'].keys():
                            data.account.avatar_id = self.settings['config']['characters'][self.settings['config']['character']]
                        else:
                    ...
                    case '.lq.Lobby.createRoom':  # 友人房 创建房间
                        ...
                        for p in data.room.persons:
                            ...
                            p.character.level = 5
                            if p.account_id == self.safe['account_id']:
                                p.avatar_id = self.settings['config']['characters'][
                    ...
                    case '.lq.FastTest.authGame':  # 进入麻将桌
                        ...
                        for p in data.players:
                            ...
                            if p.account_id == self.safe['account_id']:
                                if self.settings['config']['random_character']['enabled'] and \
                                        self.settings['config']['random_character']['pool'] != []:  # 处理随机角色
                                    ...
                                    p.character.charid = self.settings['config']['character']
                                    p.avatar_id = p.character.skin = self.settings['config']['characters'][
                                        self.settings['config']['character']]
                                p.character.exp = 0
                                if self.settings['config']['emoji']:
                                    p.character.extra_emoji.extend(
